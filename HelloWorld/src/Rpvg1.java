...
public class Rpvg1 {
    public static void main(String[] args) {
        ...
        int currentChoice = 0;
        int choiceCount; //Hilfsvariable für um die Nummerierung der Optionen bzw Eingabegefängnis
        int chosenOption; //Eingabe durch Benutzer
        int[] choiceMemoryArr = new int[10]; //an Index choiceCount wird der zugehörige Index vom Datenarray gespeichert
    ...
    do {
        ...
        for (int i = 0; i < choices.length; i++) {
            if (Integer.parseInt(choices[i][0]) == currentChoice) {
                System.out.println(choiceCount + ".) " + choices[i][2]);
                choiceMemoryArr[choiceCount] = i;
                choiceCount++;
            }
        }
        System.out.println("-".repeat(50));
        ...
        } while (!hasStopped && ( chosenOption < 1 || chosenOption > choiceCount - 1));

        if(!hasStopped) {
            //Umwandlung der Option in die zugehörige ID
            currentChoice = choiceMemoryArr[chosenOption];
            //System.out.println("Du wählst die Option mit der ID: "+currentChoice);
